...
enum class SurfaceFormat : std::uint32_t
{
  ...
  , normalized_byte_2 = GL_RG8_SNORM            ///< (Signed format) 16-bit bump-map format using 8 bits each for u and v data.
  , normalized_byte_4 = GL_RGBA8_SNORM          ///< (Signed format) 32-bit bump-map format using 8 bits for each channel.
