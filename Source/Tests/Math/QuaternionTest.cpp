...
{
    auto result = Quaternion::create_from_axis_angle({ 1.0f, 0.0f, 0.0f }, SceneR::Math::pi_over_2);
...
TEST_F(QuaternionTest, Lerp)
{
    ...
    auto a        = Quaternion::create_from_axis_angle(axis, SceneR::Math::to_radians(10.0f));
    auto b        = Quaternion::create_from_axis_angle(axis, SceneR::Math::to_radians(30.0f));
    auto t        = 0.5f;
    auto expected = Quaternion::create_from_axis_angle(axis, SceneR::Math::to_radians(20.0f));
...
TEST_F(QuaternionTest, NormalizeZeroLengthQuaternion)
{
    ...
    EXPECT_TRUE(SceneR::Math::is_nan(actual.y));
    EXPECT_TRUE(SceneR::Math::is_nan(actual.z));
    EXPECT_TRUE(SceneR::Math::is_nan(actual.w));
}
...
TEST_F(QuaternionTest, CreateFromAxisAngleOfAngle30And750)
{
    ...
    auto angle1  = SceneR::Math::to_radians(30.0f);
    auto angle2  = SceneR::Math::to_radians(750.0f);
...
TEST_F(QuaternionTest, CreateFromAxisAngleOfAngle30And390)
{
    auto axis    = Vector3 { 1.0f, 0.0f, 0.0f };
    auto angle1  = SceneR::Math::to_radians(30.0f);
    auto angle2  = SceneR::Math::to_radians(390.0f);
    auto actual1 = Quaternion::create_from_axis_angle(axis, angle1);
    auto actual2 = Quaternion::create_from_axis_angle(axis, angle2);
...
{
    auto yawAngle   = SceneR::Math::to_radians(30.0f);
    auto pitchAngle = SceneR::Math::to_radians(40.0f);
    auto rollAngle  = SceneR::Math::to_radians(50.0f);
...
TEST_F(QuaternionTest, CreateFromYawPitchRoll2)
{
    ...
    for (float yawAngle = -720.0f; yawAngle <= 720.0f; yawAngle += step)
    {
        for (float pitchAngle = -720.0f; pitchAngle <= 720.0f; pitchAngle += step)
        {
            ...
            {
                auto yawRad   = SceneR::Math::to_radians(yawAngle);
                auto pitchRad = SceneR::Math::to_radians(pitchAngle);
                auto rollRad  = SceneR::Math::to_radians(rollAngle);
...
{
    auto angle  = SceneR::Math::to_radians(180.0f);
    auto matrix = Matrix::create_rotation_y(angle)
                * Matrix::create_rotation_z(angle);
...
{
    auto a = Quaternion { SceneR::Math::nan, 0, 0, 0 };
    auto b = Quaternion { 0, SceneR::Math::nan, 0, 0 };
    auto c = Quaternion { 0, 0, SceneR::Math::nan, 0 };
    auto d = Quaternion { 0, 0, 0, SceneR::Math::nan};
